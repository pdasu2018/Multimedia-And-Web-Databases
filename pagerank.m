...
G = digraph(T);
 plot (G);
pr = centrality(G,'pagerank','MaxIterations',30,'FollowProbability',0.85);
for h = 1 : uniqueRows
    pr(h,2)=h;
end
newpr = sortrows(pr,-1);
...
% for w =1:uniqueRows
% count=0; 
% if w ==assignValues(w, 3)
%     temp(w,1) = assignValues(w, 1);
%     temp(w, 2)=assignValues(w,2);
% end 
% for p=1:uniqueRows 
% if temp(w,1)== matrixinput(p,3)&& temp(w,2)==matrixinput(p,4)
% count= count+1;
% end
% end
% temp(w,3) = count;
